...
ok('couche 2 moteur présent', t.includes('=== A. DEUX AXES'));
ok('couche 3 après le moteur', t.indexOf('COUCHE 3')>t.indexOf('=== S.') || !t.includes('COUCHE 3'));

L('\n══ CORRECTIFS DU TEST RÉEL (problèmes 1→10) ══');
ok('P1 rotation mécanique', t.includes('index+1 modulo') && t.includes('tu ne demandes jamais'));
...
ok('P8 difficulté progressive ±0,5', t.includes('jamais plus de ±1') && t.includes('INTERDIT de sauter de 2 à 8'));
ok('P9 qualité (mitose/Westphalie)', t.includes('fission binaire') && t.includes('Westphalie'));
ok('P10 mécaniques procédurales', t.includes('Vol ouvert') && t.includes('INTERDIT de juger une estimation'));

L('\n══ CAS DE TEST A→K (section 27) ══');
ok('A rotation cyclique', t.includes('index+1 modulo'));
...
ok('G pas de saut 2→8', t.includes('+1 au maximum'));
ok('H Tour Défi calibré', t.includes('un 5/5 doit être nettement plus dur'));
ok('I estimation compétitive', t.includes('CHAQUE joueur') && t.includes('plus proche'));
ok('J vol après annonce', t.includes('avant « Vol ouvert » ne rapporte rien'));
ok('K humour sans toucher au score', t.includes('mauvaise foi') && t.includes('ARBITRE ne l'));
